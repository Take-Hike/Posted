...
finity.very_dark_theme = { -- very dark
	main_container = Color3.fromRGB(22, 22, 22),
	main_containerimage = "",
	separator_color = Color3.fromRGB(63, 63, 65),
...
function finity.new(themename, gprojectName, thinProject)
	...
	local toggledKey = Enum.KeyCode.LeftShift
	...
	if string.lower(themename) == "verydark" then theme = finity.very_dark_theme end
	...
	self2.ChangeToggleKey = function(NewKey)
		...
		if not projectName then
			self2.tip.Text = "Press ' LeftShift ' to hide this menu"
		end
		...
		function category:Sector(name)
			...
			function sector:Cheat(kind, name, callback, data)
				...
				if kind then
					...
					elseif string.lower(kind) == "multipledropdown" then
						if data then
							cheat.value = {}
						end
							...
							for _, value in next, options do
								...
								button.MouseButton1Click:Connect(function()
									if cheat.dropped then
										...
										for i, v in ipairs(cheat.value) do
											if v == value then
										...
										local texitii = ""
										for i,v in pairs(cheat.value) do
											if texitii == "" then
												texitii = v
											else
												texitii = texitii..", "..v
